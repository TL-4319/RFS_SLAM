function draw_trajectory_pose (pos, quat, traj_hist, quat_hist, axis_lenght, linewidth,traj_color,bool_hold)
    %% Function to draw a trajectory
    % pos - current position
    % quat - current quaternion
    % traj_hist - position vector from intial to previous timestep
    % quat_hist - quatenion history to plot 
    ...
    if ~strcmp(traj_color,'none')
        ...
        for ii = 1:50:size(traj_hist,2)
            draw_axis(traj_hist(:,ii), quat_hist(ii,:), axis_lenght, linewidth)
        end
    end
