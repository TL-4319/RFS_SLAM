function h = plot_gaussian_ellipsoid(m, C,inten,sdwidth, npts, axh)
...
%  Revision 1.2, 26-Jun-2024
%    - Add intensity argument to plot GM for RFS filters
...
switch numel(m)
   case 2, h=show2d(m(:),C,sdwidth,npts,axh);
    case 3, h=show3d(m(:),C,inten,sdwidth,npts,axh);
   otherwise
...
%-----------------------------
function h = show3d(means, C, inten, sdwidth, npts, axh)
if isempty(npts), npts=10; end
[x,y,z] = sphere(npts);
ap = [x(:) y(:) z(:)]';
[v,d]=eig(C); 
if any(d(:) < 0)
...
cp = ones(size(z)) * inten;
h = surf(axh, xp,yp,zp, cp);