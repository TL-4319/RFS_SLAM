...
for i=2:size(time_vec,2)
    ...
    meas_reprojected = reproject_meas(truth.pos(:,i), truth.quat(i,:), meas); % This is for visualizing measurements in global space
    ...
    %% SLAM runs here
    tic
    Parlikeli = zeros (1,filter_params.num_particle);   %Pre allocate particle likelihood vector
    for par_ind = 1:size(particles,2)
        cur_particle = particles(1,par_ind);
        %% Trajectory prediction
        % Sample random noise to add in odometry measurements
        body_vel_sample = randn(3,1) .* filter_params.motion_sigma(1:3);
        body_rot_vel_sample = randn(3,1) .* filter_params.motion_sigma(4:6);
        
        % Constraint velocity noise to 2D
        body_vel_sample(2:3,:) = 0;
        body_rot_vel_sample(1:2,:) = zeros (2,1);
        
        % Particle odometry sample. Add noisy odometry measurement with
        % random noise
        body_vel_sample = odom.body_trans_vel(:,i) + body_vel_sample; 
        body_rot_vel = odom.body_rot_vel(:,i) + body_rot_vel_sample;
        
        % Predict the particle state given the sampled odometry
        ...
        % Assign pose back to particle structure
        ...
        if num_GM_in > 0
            ...
            %% Compute particle likelihood using single cluster likelihood
        ...
        %% If there are no map GM in FOV of particle. We kill the particle as it is not useful
        else
