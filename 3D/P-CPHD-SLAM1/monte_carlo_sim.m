...
% Select dataset
load ('../generated_datasets/rover_3D_8Hz.mat');
...
sensor_params.measurement_std = [0.01, 0.01, 0.01];  
...
filter_params.sensor.measurement_std = [0.01, 0.01, 0.01];
...
filter_params.cluster_max_card = 60;
filter_params.survive_prob = 0.99;
filter_params.birthGM_intensity = 0.9;             % Default intensity of GM component when birth
...
filter_params.sensor.clutter_density = 1 / ...
    filter_params.sensor.meas_area; % Uniform clutter over FOV volume
...
for ii = 1:num_run-1
    [results{ii,1},~] = p_cphd_slam1_3d_instance(dataset,sensor_params, odom_params, filter_params, draw);
end

[results{num_run,1},truth] = p_cphd_slam1_3d_instance(dataset,sensor_params, odom_params, filter_params, draw);
