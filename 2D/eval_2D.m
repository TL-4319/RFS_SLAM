...
% Iterate through sim results to find avg error
for ii = 1:size(simulation.result,1)
    ...
    for kk = 2:size(sensor_time_vec,2)
        ...
        if ii == 1 
            ...
            true_map = get_comps(simulation.truth.cummulative_landmark_in_FOV{kk,1},[1,2]);
        end
        est_map = get_comps(simulation.result{ii,1}.filter_est.map_est{kk,1}.feature_pos,[1,2]);
...
figure(1)
subplot (3,1,1)
plot (time_vec, slam_pos_error(1,:),'DisplayName',"PHD-SLAM")
hold on 
...
subplot (3,1,2)
plot (time_vec, slam_pos_error(2,:),'DisplayName',"PHD-SLAM")
hold on
...
subplot (3,1,3)
plot (time_vec, slam_pos_error(3,:),'DisplayName',"PHD-SLAM")
hold on
...
figure(2)
subplot (3,1,1)
plot (time_vec, slam_eul_error(1,:)*pi/180,'DisplayName',"PHD-SLAM")
hold on 
...
subplot (3,1,2)
plot (time_vec, slam_eul_error(2,:)*pi/180,'DisplayName',"PHD-SLAM")
hold on
...
subplot (3,1,3)
plot (time_vec, slam_eul_error(3,:)*pi/180,'DisplayName',"PHD-SLAM")
hold on
