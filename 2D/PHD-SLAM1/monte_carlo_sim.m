...
% For cartesian model, meas_vector = [x, y, z]'. Z is 0 for 2D
% For range_bearing, meas_vector = [az, el, range]', el is 0 for 2D
sensor_params.meas_model = 'cartesian'; %[cartesian, range-bearing]
sensor_params.HFOV = deg2rad(100);
sensor_params.VFOF = deg2rad(10); % For 2D sim, vertical FOV is irrelevant
sensor_params.max_range = 40;
sensor_params.min_range = 0.4;
sensor_params.detect_prob = 0.9;
sensor_params.measurement_std = [0.1, 0.1, 0.00001];
...
odom_params.motion_sigma = [0.5; 0.5; 0; 0; 0; 0.2]; 
...
filter_params.sensor.measurement_std = [0.1, 0.1, 0.00001]; % Artificially small Z measurements to 
...
filter_params.motion_model = 'odometry'; % [odometry, random-walk]
...
filter_params.adaptive_birth_dist_thres = 0.7;
...
filter_params.birthGM_cov = diag([filter_params.birthGM_std, filter_params.birthGM_std, 0.0001].^2);
filter_params.map_Q = diag([filter_params.map_std, filter_params.map_std, 0].^2);
...
for ii = 1:num_run
    results{ii,1} = phd_slam1_2d_instance(dataset,sensor_params, odom_params, filter_params);
end